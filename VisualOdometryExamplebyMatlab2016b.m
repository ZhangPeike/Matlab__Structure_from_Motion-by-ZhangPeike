...
% Detect features. 
% A non-negative scalar which specifies a threshold for selecting the strongest features. Decrease it to return more blobs.
...
% Set 'Upright'to true when you do not need the image descriptors to capture rotation information.
prevFeatures = extractFeatures(prevI, prevPoints, 'Upright', true);

% Add the first view. Place the camera associated with the first view  at the origin, oriented along the Z-axis.
viewId = 1;
...
% The width of camera's base specified as a scalar.
cameraSize = 7;
% opacity 不透明
% object = fun
...
for viewId = 3:15
    ...
    % Zhang Peike 仅利用前两帧的匹配点重构空间点，后利用此结果对当前帧做PnP
    % 修改为利用全局3D点，利用Track数据
    ...
    % Store the point matches between the previous and the current views.
    vSet = addConnection(vSet, viewId-1, viewId, 'Matches', indexPairs);    
    %tracks is an array of pointTrack objects
